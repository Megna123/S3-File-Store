	...
	public boolean PerformSRPAuthentication(String username, String password) {
		...
		try {
			...
			if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.getChallengeName())) {
				...
				JSONObject payload = CognitoJWTParser.getPayload(authresult);
				String provider = payload.get("iss").toString().replace("https://", "");

				if (provider.contains(this.userPoolID) && payload.get("aud").equals(this.clientId)
						&& ((int) (payload.get("exp")) > ts)) {
					isAuthSuccess = true;
